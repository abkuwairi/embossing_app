...
if auth_status is False:
        ...
        with um_tab:
            ...
            with tabs[2]:
                ...
                with st.form('edit_user_form'):
                    ...
                    role_opts=['management','viewer','uploader'] if role=='management' else ['admin','management','viewer','uploader']
                    if info['role'] not in role_opts: role_opts.insert(0,info['role'])
                    sel_role2=st.selectbox('نوع المستخدم',role_opts,index=role_opts.index(info['role']))
                    cpwd=st.checkbox('تغيير كلمة مرور')
                    if cpwd: new_pwd=st.text_input('كلمة المرور الجديدة',type='password')
                    if st.form_submit_button('حفظ'):
                        if sel_role2=='admin' and role!='admin':
                            ...
                            if cpwd and new_pwd: info['password']=stauth.Hasher([new_pwd]).generate()[0]
    ...
    with report_tab:
        ...
        if can_up:
            ...
            if uploaded:
                try:
                    df_new=pd.read_csv(uploaded,dtype=str) if uploaded.name.lower().endswith('.csv') else pd.read_excel(uploaded,dtype=str)
                    df_new['Load Date']=datetime.today().strftime('%Y-%m-%d')
                    df_comb=pd.concat([pd.read_excel(master_file,dtype=str),df_new],ignore_index=True) if os.path.exists(master_file) else df_new
                    df_comb.to_excel(master_file,index=False)
                    st.success('✅ تم تحديث قاعدة البيانات بنجاح.')
                except Exception as e: st.error(f'❌ خطأ أثناء رفع الملف: {e}')
            ...
            if 'Delivery Branch Code' not in df_all.columns:
                ...
                for br in sorted(df_all['Delivery Branch Code'].unique()):
                    ...
                    with st.expander(f'📌 فرع {br}'):
                        ...
                        if can_dn:
                            buf=io.BytesIO()
                            with pd.ExcelWriter(buf,engine='xlsxwriter') as w: df_br.to_excel(w,index=False)
                            buf.seek(0)
        ...
        else: st.info('ℹ️ لا توجد بيانات بعد.')
