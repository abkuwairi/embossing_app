...
# Prepare Streamlit-Authenticator with active users
auth_users = {
...
if auth_status is False:
    ...
    # Define permissions for upload/download
    can_upload = role in ['admin', 'management', 'uploader']
    can_download = role in ['admin', 'management', 'uploader']

    # Upload section
    if can_upload:
        uploaded_file = st.file_uploader('📁 رفع تقرير البطاقات', type=['xlsx'])
        if uploaded_file:
            try:
            ...
            for branch in sorted(df_all['Delivery Branch Code'].unique()):
                ...
                with st.expander(f'📌 فرع {branch}'):
                    ...
                    if can_download:
                        ...
                        buf.seek(0)
                        st.download_button(f'⬇️ تحميل فرع {branch}', buf, f'branch_{branch}.xlsx', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    else:
